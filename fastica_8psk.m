...
for i = 1:length(dB_noise)
    ...
    aa(:,:,i) = B'*Q; 
    psk_8_ICAedS(:,:,i) = aa(:,:,i)*mixed_data_noised(:,:,i);   %得到分离后的输出信号
    psk_8_rmsEVM_separated_f1(i,:) = evm(source_data_unnoised.',psk_8_ICAedS(:,:,i).');
    psk_8_rmsEVM_separated_f1(i,:) = 20*log10(0.01*psk_8_rmsEVM_separated_f1(i,:));
    rmsEVM_subtract_f1(i,:) = psk_8_rmsEVM_separated_f1(i,:) - psk_8_rmsEVM_unseparated(i,:);
end
...
%% %%%%%%%% 绘制第1张 alpha-EVM图 %%%%%%%%%%
figure(1);
plot(dB_noise',psk_8_rmsEVM_unseparated(:,2),'-or');hold on;
grid on;
grid minor;
plot(dB_noise',psk_8_rmsEVM_separated_f1(:,2),'-ok');
...
num = length(dB_noise);
psk_8_aa_2 = zeros(num,2);
psk_8_aa_3 = zeros(2,2,num);
for i = 1:num
%     aa_3(:,:,i) = real (aa(:,:,i)*mix_matrix); 
    psk_8_aa_3(:,:,i) = aa(:,:,i)*mix_matrix; 
    psk_8_aa_2(i,1) = psk_8_aa_3(2,2,i); 
    psk_8_aa_2(i,2) = psk_8_aa_3(2,1,i);
...
plot(dB_noise',20*log10(abs(psk_8_aa_2(:,1))./abs(psk_8_aa_2(:,2))),'-og');
hold on;
...
for i =1:length(dB_noise)
    ...
    psk_8_dB_SINR_unseparation(i,:) = 10*log10(P_signal./P_ganrao_plus_noise_unseparation(i,:));  

    % 计算 分离的SINR    f1
    interference_noise_separation_f1(:,:,i) = psk_8_ICAedS(:,:,i) - source_data_unnoised;
    P_ganrao_plus_noise_separation_f1(i,:) = sum((interference_noise_separation_f1(:,:,i) .*conj(interference_noise_separation_f1(:,:,i) )).');    
    psk_8_dB_SINR_separation_f1(i,:) = 10*log10(P_signal./P_ganrao_plus_noise_separation_f1(i,:)); 
...
figure(3);
plot(dB_noise',psk_8_dB_SINR_unseparation(:,2),'-.pr');
hold on;grid on;
grid minor;
plot(dB_noise',psk_8_dB_SINR_separation_f1(:,2),'-pg');
...
%    PI_unseparation = 1/2*4*ones(1,length(dB_noise))*0.1^0.5;
    ...
    psk_8_PI_separation_1(i) = 1/2*(abs(psk_8_aa_3(1,2,i))/abs(psk_8_aa_3(1,1,i)) +abs(psk_8_aa_3(2,1,i))/abs(psk_8_aa_3(2,2,i)) + ...
        abs(psk_8_aa_3(2,1,i))/abs(psk_8_aa_3(1,1,i)) + abs(psk_8_aa_3(1,2,i))/abs(psk_8_aa_3(2,2,i)) );

end

%%%%%%%%%% 绘制第5张 SNR-PI(performance index)图 %%%%%%%%%%
figure(5);
plot(dB_noise,PI_unseparation,'-ko',dB_noise,psk_8_PI_separation_1,'-bp');
hold on;
...
% save psk_8_0.1_20db_data.mat psk_8_ICAedS psk_8_rmsEVM_separated_f1 psk_8_rmsEVM_unseparated ...
%     psk_8_aa_2 ...
%     psk_8_dB_SINR_unseparation psk_8_dB_SINR_separation_f1 ...
%     psk_8_PI_separation_1
...
subplot(122);plot(psk_8_ICAedS(1,:,6),'.'); hold on;
plot(source_data_unnoised(1,:),'xr') % axis([-2 2 -1 1]); 
