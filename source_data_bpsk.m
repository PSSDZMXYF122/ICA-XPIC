...
% 
% 
% bpsk_rmsEVM_unseparated = zeros(length(dB_noise),2);
% 
% 
% for i = 1:length(dB_noise)
%     mixed_data_noised(:,:,i) = awgn(mixed_data_unnoised,dB_noise(i),'measured');   %给信号加噪声
%     bpsk_rmsEVM_unseparated(i,:) = evm(source_data_unnoised.',mixed_data_noised(:,:,i).');
%     bpsk_rmsEVM_unseparated(i,:) = 20*log10(0.01*bpsk_rmsEVM_unseparated(i,:)) ;
%     % rmsEVM定义为平均误差矢量功率与平均基准功率的比值的平方根
% %     disp(['混合信号加噪声后的evm分别为：  ' num2str(rmsEVM_unseparated(i,:)) ' dB']);
% end
% 
% save source_data_bpsk.mat bpsk_rmsEVM_unseparated mixed_data_noised sn_unnoise gn_unnoise  dB_noise source_data_unnoised alpha_mix mix_matrix
% 
...
%     alpha_mix = 10^(-20/20);
%     alpha_mix = 10^(-10/20);
    alpha_mix = 10^(-10/20)*exp(sqrt(-1)*pi/100);
